            ...
            Timer.after(1.5, function()
                ...
                :finish(function()
                    ...
                    if self.playerPokemon.currentExp > self.playerPokemon.expToLevel then
                        ...
                        local newHP = self.playerPokemon.HP + HPIncrease
                        local newAttack = self.playerPokemon.attack + attackIncrease
                        local newDefense = self.playerPokemon.defense + defenseIncrease
                        local newSpeed = self.playerPokemon.speed + speedIncrease

                        gStateStack:push(BattleMessageState('Congratulations! Level Up!',
                        function()

                            -- push the LeveulUpMenuState
                            gStateStack:push(LevelUpMenuState(self))
                        end))
                            -- show the updated stat values
                            gStateStack:push(LevelUpMessageState(' New HP = Current (' .. tostring(self.playerPokemon.HP) .. ') + Increase (' .. tostring(HPIncrease) .. ') = ' .. tostring(newHP)
                                ..'\n New Attack = Current (' .. tostring(self.playerPokemon.attack) .. ') + Increase (' .. tostring(attackIncrease) .. ') = ' .. tostring(newAttack)
                                ..'\n New Defense = Current (' .. tostring(self.playerPokemon.defense) .. ') + Increase (' .. tostring(defenseIncrease) .. ') = ' .. tostring(newDefense)
                                ..'\n New Speed = Current (' .. tostring(self.playerPokemon.speed) .. ') + Increase (' .. tostring(speedIncrease) .. ') = ' .. tostring(newSpeed),
                                function()
                                    gStateStack:pop()
                                    self:fadeOutWhite()
                                end, 1, VIRTUAL_WIDTH - LEVEL_UP_MENU_WIDTH, 16, LEVEL_UP_MENU_WIDTH, LEVEL_UP_MENU_HEIGHT))
                    else
                        self:fadeOutWhite()
                    end
                end)
